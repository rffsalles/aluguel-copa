...
def check_typescript_coverage(project_path: Path) -> dict:
    ...
    ts_files = [f for f in ts_files if f.relative_to(project_path).parts[0] == 'src' and '.d.ts' not in str(f)]
    
    if not ts_files:
        return {'type': 'typescript', 'files': 0, 'passed': [], 'issues': ["[!] No TypeScript files found"], 'stats': stats}
    
    for file_path in ts_files[:200]:  # Limit to 200 representative files
        try:
