...
class UXAuditor:
    ...
    def audit_file(self, filepath: str) -> None:
        ...
        # Try to make path relative for better output
        try:
            cwd = os.getcwd()
        ...
        form_elements_raw = re.findall(r'<input[^>]*>|<select[^>]*>|<textarea[^>]*>', content, re.IGNORECASE)
        # Filter out hidden inputs which don't need labels
        form_elements = [el for el in form_elements_raw if 'type="hidden"' not in el.lower() and "type='hidden'" not in el.lower()]
        complex_elements = len(form_elements)
    ...
    def audit_directory(self, directory: str) -> None:
        ...
        for root, dirs, files in os.walk(directory):
            dirs[:] = [d for d in dirs if d not in {'node_modules', '.git', 'dist', 'build', '.next', 'playwright-report'}]
            for file in files:
