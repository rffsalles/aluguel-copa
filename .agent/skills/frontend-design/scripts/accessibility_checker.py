...
def find_html_files(project_path: Path) -> list:
    ...
    skip_dirs = {'node_modules', '.next', 'dist', 'build', '.git', 'components'}
    ...
    for pattern in patterns:
        for f in project_path.glob(pattern):
            if not any(skip in f.parts for skip in skip_dirs):
                name = f.name.lower()
                if f.suffix.lower() == '.html' or name in ['page.tsx', 'page.jsx', 'layout.tsx', 'layout.jsx', 'index.tsx', 'index.jsx']:
                    files.append(f)
...
def main():
    ...
    # Accessibility issues are important warnings but do not block the build
    passed = True
