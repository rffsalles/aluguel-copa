...
def find_api_files(project_path: Path) -> list:
    ...
    # Exclude node_modules, tests, generated, hooks, etc.
    return [f for f in files if not any(x in str(f) for x in ['node_modules', '.git', 'dist', 'build', '__pycache__', 'tests', 'generated', 'hooks', 'manual-scripts'])]
