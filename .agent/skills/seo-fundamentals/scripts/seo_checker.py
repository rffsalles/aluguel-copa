...
def check_page(file_path: Path) -> dict:
    ...
    # Detect if this is a layout/template file (has Head component or NextJS metadata export)
    is_layout = '<Head' in content or '<head>' in content.lower() or 'export const metadata' in content or 'export async function generateMetadata' in content
    ...
    try:
        import os
        cwd = os.getcwd()
        filename = os.path.relpath(file_path, cwd)
    except Exception:
